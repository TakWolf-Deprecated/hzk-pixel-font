...
def make_preview_image_file(font_config: FontConfig):
    ...
    image = Image.new('RGBA', (font_config.size * 35, font_config.size * 11), (255, 255, 255, 255))
    draw = ImageDraw.Draw(image)
    draw.text((font_config.size, font_config.size), '汉字库像素字体 / HZK Pixel Font', fill=text_color, font=font)
    draw.text((font_config.size, font_config.size * 3), '我们每天度过的称之为日常的生活，其实是一个个奇迹的连续也说不定。', fill=text_color, font=font)
